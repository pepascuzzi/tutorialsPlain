...
The original tutorial were written with the **learnr package**.  This version is plain Rmarkdown.  You should run through this tutorial chunk by chunk.  If you introduce errors that you cannot fix, please download a new copy.  

Note, there are chunks that were meant to create errors in the original tutorials.  The code in these chunks has been "commented-out" so that this document will **Knit**.  In addition, there are chunks that are exercises.  Some code may be "commented-out" in these chunks as well for the same reason.
